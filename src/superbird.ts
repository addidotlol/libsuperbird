...
import { SUPERBIRD_PARTITIONS, type PartitionName } from './partitions.js';
...
export class Superbird {
  ...
  private partitionTableLoaded = false;
  ...
  static async connect(options: ConnectOptions = {}): Promise<Superbird> {
    const onStatus = options.onStatus;
    let usb = options.device ?? (await Superbird.pickDevice());

    const mode = Superbird.modeOf(usb);
    if (mode === 'normal') {
      throw new SuperbirdError(
        'wrong-mode',
        'device is booted normally - power it on while holding buttons 1 & 4 to enter USB mode',
      );
    }
    ...
    if (mode === 'usb') {
      if (!options.bl2 || !options.bootloader) {
        throw invalidOperation(
          'device is in USB mode and needs a bl2 boot: pass `bl2` and `bootloader` binaries to connect()',
        );
      }
      onStatus?.('connecting');
  ...
  private static async pickDevice(): Promise<USBDevice> {
    if (!('usb' in navigator)) {
  ...
  private static async waitForBurnDevice(timeoutMs: number): Promise<USBDevice> {
    const deadline = performance.now() + timeoutMs;
    while (performance.now() < deadline) {
      const devices = await navigator.usb.getDevices();
      const found = devices.find(
        d => d.vendorId === proto.VENDOR_ID && d.productId === proto.PRODUCT_ID && d.productName !== 'GX-CHIP',
      );
    ...
    throw new SuperbirdError(
      'not-found',
      'device did not reappear in burn mode - it may need to be re-paired via requestDevice()',
    );
  }
  ...
  private async open(): Promise<void> {
    ...
    if (!alternate) throw invalidOperation('interface 0 has no alternate setting');
    ...
    for (const endpoint of alternate.endpoints) {
      ...
      else endpointOut = endpoint.endpointNumber;
    }
  ...
  private ensureOpen(): void {
    if (!this.opened) throw invalidOperation('device is not open - use Superbird.connect()');
  }

  private async controlOut(request: number, value: number, index: number, data?: Uint8Array): Promise<void> {
    this.ensureOpen();
  ...
  private async controlIn(request: number, value: number, index: number, length: number): Promise<Uint8Array> {
    this.ensureOpen();
    const result = await withTimeout(
      this.usb.controlTransferIn({ requestType: 'vendor', recipient: 'device', request, value, index }, length),
      this.config.commandTimeoutMs,
      `control transfer ${hex(request)}`,
    );
  ...
  private async bulkOut(data: Uint8Array, timeoutMs = this.config.bulkTimeoutMs): Promise<number> {
    this.ensureOpen();
    const result = await withTimeout(
      this.usb.transferOut(this.endpointOut, data as BufferSource),
      timeoutMs,
      'bulk write',
    );
    if (result.status !== 'ok') {
  ...
  private async bulkIn(length: number, timeoutMs = this.config.commandTimeoutMs): Promise<Uint8Array> {
    this.ensureOpen();
  ...
  async identify(): Promise<string> {
    ...
    if (data.length !== 8) throw invalidOperation('failed to read identify data');
    return new TextDecoder().decode(data);
  }

  async writeSimpleMemory(address: number, data: Uint8Array): Promise<void> {
    if (data.length > 64) throw invalidOperation('maximum size of 64 bytes');
  ...
  async readSimpleMemory(address: number, length: number): Promise<Uint8Array> {
    if (length === 0) return new Uint8Array(0);
    if (length > 64) throw invalidOperation('maximum size of 64 bytes');
    const data = await this.controlIn(proto.REQ_READ_MEM, address >>> 16, address & 0xffff, length);
    if (data.length !== length) throw invalidOperation('incomplete read');
    return data;
  }
  ...
  async run(address: number, keepPower = true): Promise<void> {
  ...
  async bulkcmd(command: string): Promise<string> {
  ...
  async writeLargeMemory(
    address: number,
    data: Uint8Array,
    blockLength = this.config.transferBlockSize,
    appendZeros = true,
  ): Promise<void> {
    let payload = data;
    const remainder = data.length % blockLength;
    if (remainder !== 0) {
      if (!appendZeros) throw invalidOperation('data must be a multiple of block length');
  ...
  async bl2Boot(bl2: BinarySource, bootloader: BinarySource): Promise<void> {
    ...
    for (let iteration = 0; iteration < 50; iteration++) {
      ...
      if (length === prevLength && offset === prevOffset) return;
      prevLength = length;
      prevOffset = offset;
  ...
  private async getBootAmlc(): Promise<{ length: number; offset: number }> {
    ...
    if (buf.length < proto.AMLC_AMLS_BLOCK_LENGTH) throw invalidOperation('no amlc data received');

    const tag = new TextDecoder().decode(buf.subarray(0, 4));
    if (tag !== 'AMLC') throw invalidOperation(`invalid amlc request: ${tag}`);
  ...
  private async writeAmlcData(offset: number, data: Uint8Array): Promise<void> {
    await this.controlOut(
      proto.REQ_WRITE_AMLC,
      Math.floor(offset / proto.AMLC_AMLS_BLOCK_LENGTH),
      data.length - 1,
    );
    ...
    while (remaining > 0) {
      ...
      await retry(3, 100, async () => {
        ...
        if (written !== blockLength) throw new SuperbirdError('usb', `incomplete bulk write: ${written}/${blockLength}`);
      });
    ...
    const ackBuf = await retry(3, 100, async () => {
      ...
      if (buf.length < 4) throw invalidOperation(`short ack read: ${buf.length} bytes`);
      return buf;
    });

    const ack = new TextDecoder().decode(ackBuf.subarray(0, 4));
    if (ack !== 'OKAY') throw invalidOperation(`invalid amlc data write ack: ${ack}`);
  }

  async writeAmlcDataPacket(seq: number, amlcOffset: number, data: Uint8Array): Promise<void> {
    let offset = 0;
    while (offset < data.length) {
  ...
  private async ensurePartitionTable(): Promise<void> {
    if (this.partitionTableLoaded) return;
    await this.bulkcmd('amlmmc part 1');
    this.partitionTableLoaded = true;
  }

  async validatePartitionSize(name: PartitionName): Promise<number> {
    ...
    const tryRead = (size: number) =>
      this.bulkcmd(
        `amlmmc read ${name} ${hex(proto.ADDR_TMP)} ${hex(size - proto.PART_SECTOR_SIZE)} ${hex(proto.PART_SECTOR_SIZE)}`,
      );
    ...
    try {
      ...
      if (info.sizeAlt === undefined) throw error;
  ...
  async writeUserArea(lba: number, data: StreamSource, options: WriteOptions = {}): Promise<void> {
    const { reader, size } = await resolveStream(data);
  ...
  async writeBootPartition(hwpart: 1 | 2, data: BinarySource): Promise<void> {
    ...
    if (bytes.length > this.config.stageChunkSize) {
      throw invalidOperation(
        `boot partition payload ${bytes.length} bytes exceeds single-transfer cap ${this.config.stageChunkSize}`,
      );
    }
  ...
  async writeEnv(env: string, options: { save?: boolean } = {}): Promise<void> {
    if (!isAscii(env)) throw invalidOperation('env data must be ascii');
    ...
    if (options.save) await this.bulkcmd('saveenv');
  }

  private async stagedWrite(
    reader: ByteReader,
    totalBytes: number,
...
function amlcChecksum(data: Uint8Array): number {
